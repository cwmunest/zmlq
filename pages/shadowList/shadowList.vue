<template>
	...
	<view class="pageHomeWrap">
		<!-- 导航 -->
		<uni-nav-bar left-icon="back" title="光影效果" @clickLeft="goBack"></uni-nav-bar>
		<view class="listBox">
			<block v-for="(item,index) in list" :key="index">
				<item-shopping :imgUrl="item.image" :title="item.name" :desc="item.intro" :price="item.price" @onClick="goDetail(item.id)"></item-shopping>
			</block>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			goDetail(id) {
				linkTo('../shadowDetail/shadowDetail?id='+id)
			},
