<template>
	...
	<view class="pageHomeWrap">
		<!-- 导航 -->
		<uni-nav-bar left-icon="back" title="酒店房型" @clickLeft="goBack"></uni-nav-bar>
		<view class="listBox">
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			params.hotelsId = options.id
			_this.queryAppHotelsRoomTypeList(_this)
		},
		// 上拉加载更多
		onReachBottom() {
		...
		methods: {
			...
			queryAppHotelsRoomTypeList: (_this) => {
				const paramsPayload = {
					apixkey: "queryAppHotelsRoomTypeList",
